    ...
    while True:
        ...
        if not in_event:
            ...
            if start_counter >= start_trigger_frames:
                ...
                # 關鍵修正：
                # 1) 先回推到「連續命中開始的第一幀」
                # 2) 再往前扣掉 pre_event_sec 對應的 frame 數
                # 這樣 pre_event_sec 會真正影響輸出片段的起點，而不是只靠 buffer 長度估算
                ...
                next_long_stay_shot_frame = frame_idx + max(1, int(round(LONG_STAY_SCREENSHOT_INTERVAL_SEC * fps)))
        else:
            if inside_present:
                last_inside_frame = frame_idx

                if export_screenshots and next_long_stay_shot_frame is not None and frame_idx >= next_long_stay_shot_frame:
                    ok_save, shot_path = try_save_screenshot(
                        logs=logs,
                        screenshot_out_dir=screenshot_out_dir,
                        base_name=base_name,
