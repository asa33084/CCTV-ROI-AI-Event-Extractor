...
$env:CCTV_ROI_STREAM_WORKERS="1"    # >1 時每個 stream 獨立派工；同一 camera 的 streams 也可平行處理
...
`CCTV_ROI_STREAM_WORKERS` 會把每個連續 stream 當成獨立工作項目，即使 streams 屬於同一 camera 也能由不同 process 平行處理。每個 worker 會獨立載入 YOLO / LPR 模型與 tracker，stream 之間不會共用追蹤狀態。此值是最大同時 worker 數；CPU 模式建議先從 `2` 開始測。若同時設定 `CCTV_ROI_TORCH_THREADS` / `CCTV_ROI_OPENCV_THREADS`，每個 worker 都會套用這些 thread 數，總量過高可能造成 CPU oversubscription。Debug track 預覽也可在 parallel 模式下使用，但影格需要跨 process 傳回 Qt，開啟時會有額外 IPC 負擔。
